...
/* Internal use only. */
#define ARRAY_HEADER_SIZE \
    (sizeof(void *) < 8 ? 8 : sizeof(void *))
typedef struct {
    /*
...
{
    return (uchar *)array->data + ARRAY_HEADER_SIZE;
}

/* Internal use only. */
array_t *array_get_ptr(void *array_data)
{
    return (array_t *)(*(uintptr_t *)((uchar *)array_data - ARRAY_HEADER_SIZE));
}
...
void *array_new_func(int size, int item_size)
{
    ...
    size_t allocate = (size_t)ARRAY_HEADER_SIZE + size * item_size;
...
/* Use: array_fill(array, value). */
#define array_fill(array_data, value)                                       \
    do {                                                                    \
        for (int i = 0; i < array_size(array_data); i++) {                  \
            array_data[i] = value;                                          \
        }                                                                   \
    } while(0)
